<?php
	...
	if( isset($_POST['deleter']) && $_POST['deleter'] == 'Delete Registration' && !empty($_POST['s']) ):
		...
		foreach( $_POST['s'] as $k=>$v ) {
			...
			DELETE FROM students_reg WHERE std_id = '.$std_id.' && season='.$season.' && yearsession = "'.$ysession.'" && level_id = '.$level.';
			DELETE FROM registered_semester WHERE std_id = '.$std_id.' && ysession = "'.$ysession.'" && rslevelid = '.$level.';
			DELETE FROM `students_results` WHERE `std_id` = '.$std_id.' && period='.$season.' && level_id = '.$level.' && std_mark_custom2 = "'.$ysession.'";';
...
//			//mysqli_close($GLOBALS['connect']);
		...
		/*	header('Location: viewregprobstudent.php?'.$query_string);
	...
	mysqli_query( $GLOBALS['connect'], 'DELETE FROM students_reg WHERE std_id = '.$std_id.' && season="'.$season.'" && yearsession = "'.$ysession.'" && level_id = '.$level.'' ) or die("one");	
		...
		header('Location: viewregdelaystudent.php?'.$query_string);
		header('HTTP/1.1 301 Moved Permanently');
		exit('Zoom');
