...
<form action="form_maxregcourses.php" method="post" autocomplete="off" onsubmit="return chk();" name="register">
...
?>
<div style=" width:1000px;margin:0 auto; background:#CCC" class="block">
<table border="0" cellpadding="0" cellspacing="0" >
  <tr>

  	<?php 
if( $_SESSION['myprogramme_id'] == 7 ) { ?>
	<td width="" class="td"><p>Month Of Entry</p>				
	<select name="month" id="month">
      <option value="">Month Of Entery</option>
...
<table border="0" cellpadding="0" cellspacing="0" style="background:#F6f6f6;margin:0 auto; font-size:11px;">
  <tr>
     ...
     <?php
      if($_SESSION['myprogramme_id']==7){
     echo'<td width="" class="td">
<span id="result"></span>
      <p>Entry Year </p><label for="select2"></label>
    ...
    <td width="" class="td"><p>Entry Year </p><label for="select2"></label>
      <select name="yearsession" id="yearsession">
 ...
 echo'</select></td>';
    ...
    <td width="" class="td"><p>Student Department</p><label for="select2"></label>
		<?php
			$where = '';
			$disable = '';
    ...
    <td width="" class="td" style="padding-right:20px"><p>Course Department</p><label for="select2"></label>
      ...
      <input type="hidden" name="prog" id="prog" value="<?php echo $_SESSION['myprogramme_id']; ?>"></td>

  </tr>
</table>
<div class="block" style="margin:0 auto; width:1050px;">
<div id="s" style=" width:600px;margin:0 auto;padding-bottom:10px;text-align:center; float:left; background:#F6F6F6" class="block">
...
<div id="c" style=" width:444px; float:left; background:#F6F6F6;text-align:center; padding-bottom:10px;" class="block">
    ...
    <?php
     ...
     echo'<span id="result"></span>
      <p>Session </p><label for="select2"></label>
...
function lss( a, id ) {
	...
	switch(v) {
		case 'Student Lists':
			if( xid("sl").value=='' || xid("sd").value=='' || xid("fd").value=='' ){
				alert("Student Level , Department and Field Of Student must be selected");
				return;
			}
			if(xid('prog').value == 7)
			{
			if( xid("month").value==''){
				alert("Student Entery month must be selected");
				return;
			}
		}
			...
			ajax.setVar("fos", xid("fd").value);
			ajax.setVar("yearsession", xid("yearsession").value );
			if(xid('prog').value == 7)
			{
			ajax.setVar("month", xid("month").value );
		}
		break;
		case 'Courses Lists':
			if( xid("cl").value=='' ){
			...
			if( xid("c_fd").value=='' ){
				alert("Field of Studies must be selected");
				return;
			}		
			ajax.setVar("a", 2);
			ajax.setVar("level", xid("cl").value);
			ajax.setVar("dept", xid("cd").value);
			ajax.setVar("c_fos", xid("c_fd").value);
			ajax.setVar("cyear", xid("cyear").value);
		break;
	}
	ajax.requestFile = "list.php";
...
function chk() {
	...
	} else if( typeof(document.forms["register"].elements["stds[]"]) == '' ) {
...
type: "POST",
data: "data=" + $(this).val(),
url: "get_course_reg_session_sandwich.php",
