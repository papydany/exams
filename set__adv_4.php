...
<?php
	...
	echo '<p style="font-size:16px; padding:7px 4px 4px; font-weight:700;">',$level_titles[ $level ],' ( Session - ',$session,' )</p>';
	...
	if( !empty($sls) ) {
		...
		foreach( $sls as $std_id=>$v ):
			...
			'<div style="font-size:12px; padding:3px; display:block; overflow:hidden"><p style="width:100px; background:#EEF2F7; text-align:center; float:left">',$matric_no,'</p> <p style="float:left;text-align:center; width:80%"><b>',$surname,',</b> ',$othername,'</p></div>';	
...
TABLESS;
					...
					foreach( $merge as $th ) {
						if( in_array($th, $disable_keys) ) {
							...
							echo '<td>',
								'<input ',$disabled,' name="d[',$std_id,'~',$matric_no,'~',$surname,'~',$othername,'~',$th['thecourse_id'],']" type="checkbox" value="" />',
							'</td>';
						} else {
							echo '<td>',
