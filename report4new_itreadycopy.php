...
<title>Undergraduate IT Result List</title>
...
$sqlsum = "SELECT stdresult_id,matric_no,students_results.std_mark_custom2 FROM students_results
...
while($ccurow = mysqli_fetch_assoc($resultcusum1)){
	...
	$custd=$custd.",".$ccurow['stdresult_id'];
		...
		$querydsum = "SELECT sum(cu) as sumcu FROM students_results where matric_no = '".$rowcount['matric_no']."' AND stdresult_id IN (".substr($custd,1,strlen($custd)).")";
