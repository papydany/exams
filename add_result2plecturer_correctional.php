...
$querycourse = "select * from all_courses where thecourse_id = $cccode[0] and level_id = ".$_GET['s_level']." and course_custom5 = ".$_GET['s_session'];
//echo $querycourse;

$resultcourse = mysqli_query( $GLOBALS['connect'],  $querycourse ) or die( "<a href=\"snd_msg.php?m=".mysqli_error($GLOBALS['connect'])."\" style=\"color:#039; font-weight:700; font-family:'Lucida Console', Monaco, monospace\">Please ( R e p o r t&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;E r r o r )</a>" );

$mycourse = mysqli_fetch_assoc($resultcourse);
/*
echo $mycourse['thecourse_id']."<br />";
echo $mycourse['course_code']."<br />";
echo $mycourse['programme_id']."<br />";
echo $mycourse['faculty_id']."<br />";
echo $mycourse['department_id']."<br />";
echo $mycourse['level_id']."<br />";
echo $mycourse['course_semester']."<br />";
echo $mycourse['course_custom2']."<br />";
*/


$querymyfos = "select department_id from all_courses where faculty_id = ".$mycourse['faculty_id']. " group by department_id";
//echo $querymyfos;
 $resultfos = mysqli_query( $GLOBALS['connect'], $querymyfos ) or die( "<a href=\"snd_msg.php?m=".mysqli_error($GLOBALS['connect'])."\" style=\"color:#039; font-weight:700; font-family:'Lucida Console', Monaco, monospace\">Please ( R e p o r t&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;E r r o r )</a>" );
 
 while($fosrow = mysqli_fetch_assoc($resultfos)){
	 
	 $myfos=$myfos.",".$fosrow['department_id'];
	 
 }
 //echo $myfos;


$querymydep = "SELECT * FROM all_courses 
where faculty_id = ".$mycourse['faculty_id']. "
and department_id IN (".substr($myfos,1,strlen($myfos)).") 
and level_id = ". $_GET['s_level']. " 
and course_custom5 = ". $_GET['s_session']. " 
group by course_custom2
order by department_id";

//echo $querymydep."<br /><br />";

 $resultdep = mysqli_query( $GLOBALS['connect'], $querymydep ) or die( "<a href=\"snd_msg.php?m=".mysqli_error($GLOBALS['connect'])."\" style=\"color:#039; font-weight:700; font-family:'Lucida Console', Monaco, monospace\">Please ( R e p o r t&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;E r r o r )</a>" );
 
 while($deprow = mysqli_fetch_assoc($resultdep)){
	 
	$myfos2=$myfos2.",".$deprow['course_custom2'];
...
$query = 'SELECT course_title, course_code FROM all_courses WHERE thecourse_id = '.$cccode[0].' LIMIT 1';
//echo $query;
...
<table width="100%" border="0" align="center" cellspacing="0" cellpadding="10" id=title>
  <tr>
	...
	<?php
		echo $mycourse['course_semester']." "; 
		echo $mycourse['programme_id'] == 7 ? $_GET['s_session'] : $_GET['s_session'].'/'.($_GET['s_session'] + 1); ?> 
...
<?php
		...
		FROM `dept_options`, course_reg LEFT JOIN students_profile ON (course_reg.std_id = students_profile.std_id) WHERE `dept_options`.`dept_id` = students_profile.`stddepartment_id` && `dept_options`.`do_id` = `students_profile`.stdcourse && `course_reg`.clevel_id = $_GET[s_level] && course_reg.thecourse_id ='".$cccode[0]."' AND course_reg.csemester $s_sem AND course_reg.cyearsession = '$_GET[s_session]' && course_reg.course_season = '$season' && students_profile.stdfaculty_id = '".$mycourse['faculty_id']."' && students_profile.stddepartment_id = '".$mycourse['department_id']."' && `dept_options`.do_id IN (".substr($myfos2,1,strlen($myfos2)).")
...
echo <<<BOTTOM
	...
	<a href="add_result_lecturer.php?s_session=$_GET[s_session]&s_semester=$_GET[s_semester]&&s_faculty=$_GET[s_faculty]&&s_level=$_GET[s_level]&Submit=Continue" class="ibutton" style="float:left; font-size:12px; color:#333; text-decoration:none;">< go back</a>
BOTTOM;
