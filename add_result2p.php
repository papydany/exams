...
<table width="100%" border="0" align="center" cellspacing="0" cellpadding="10" id=title>
  <tr>
    <td class="hder"><b>Student's Result Management (<?php echo $_SESSION['myprogramme_id'] == 7 ? ' Level '.substr(GetlLevel($_GET['s_level']),0,1) : ' Level '.GetlLevel($_GET['s_level']); ?> )</u></b><br/>
	<?php
...
<?php
		echo '<form name="form1" method="post" action="result_insert.php" autocomplete="off" onSubmit="return OnSubmitForm(this);">';		
?>
<?php
	...
	/*    pagination code    */
		...
		$query= "SELECT count(*) FROM `dept_options`, course_reg LEFT JOIN students_profile ON (course_reg.std_id = students_profile.std_id) WHERE `dept_options`.`dept_id` = students_profile.`stddepartment_id` && `dept_options`.`do_id` = `students_profile`.stdcourse && `course_reg`.clevel_id = $_GET[s_level] && course_reg.thecourse_id ='$cs_code' AND course_reg.csemester $s_sem AND course_reg.cyearsession = '$_GET[yearsession]' && course_reg.course_season = '$season' && students_profile.stdfaculty_id = '$_SESSION[myfaculty_id]' && students_profile.stddepartment_id = '$_SESSION[mydepartment_id]' && `dept_options`.do_id =".$fos."
...
if( 0!=mysqli_num_rows ($result3) ) {
	...
	mysqli_free_result($result3);
		...
		echo <<<TH
		  <tr style="">
		  <th width="3%"> </th>
		  <th width="3%"><strong>S/N</strong></th>
		  ...
		  <th width="10%"><b>Counts In GPA</b></th>
...
TH;
	foreach( $list as $k=>$v ) {
		...
		foreach( $v as $kk ) {
			...
			std_mark_custom2 = '$_GET[yearsession]' AND stdcourse_id = '$course_id' && period = '$season' ";
			...
			if ( $std_id ) {
				...
				$grade_disabled = isset($_SESSION['myedit_permission']) && $_SESSION['myedit_permission'] == 1  ? '' : 'readonly="readonly"';
				echo '<tr style="background:',$bgcolor,'">',
						...
						if ( $num2ac == 0 ) {
								echo <<<TD
										...
										<input name="std_mark[$i]" style=" background:#f0f0f0; width:50px;" type="text" id="d$i" onChange="if (this.value!='') document.getElementById('check[$i]').checked=true" value="$std_mark" size="5" maxlength="5">
...
TD;
								...
								echo <<<TD
										...
										<input name="std_mark[$i]" style=" background:#f0f0f0;width:50px;" readonly="readonly" type="text" id="d$i" onChange="if (this.value!='') document.getElementById('check[$i]').checked=true" value="$std_mark" size="5" maxlength="5">
										<input name="std_id[$i]" type="hidden" value="$std_id">
										<input name="cos_id[$i]" type="hidden" value="$course_id">
										<input name="matric_no[$i]" type="hidden" value="$matric_no">
										</td>
...
BB;
	...
	echo '<input name="certificate" type="hidden" value="',basename($_SERVER['PHP_SELF']),'?',$_SERVER['QUERY_STRING'],'">',
		 ...
		 '<input type="hidden" name="session_year" value="',$_SESSION['yearsession'],'" />',
