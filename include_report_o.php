...
*/	
	...
	$sql = 'SELECT stdcourse_id, std_grade FROM students_results WHERE level_id !=0 && std_mark_custom2 = "'.$s.'" && std_id = '.$s_id.' && std_cstatus = "yes" && period = "NORMAL" && std_grade != "N" && stdcourse_id IN ( SELECT thecourse_id FROM course_reg WHERE std_id = '.$s_id.' && std_mark_custom2 = "'.$s.'")';
	// 
...
function get_crunit ( $stdcourseid, $s ) {
	$sql = 'SELECT course_unit FROM all_courses WHERE thecourse_id = '.$stdcourseid.' && course_custom5 = '.$s.' ORDER BY 1 ASC LIMIT 1';
	$r = mysqli_query($GLOBALS['connect'], $sql );
	//return mysqli_num_rows($r);
...
function result_check_pass_sessional($l, $std_id, $s, $cgpa, $take_ignore=false)
{	$fail=''; $pass='';$c=0;// echo '['.$s.']';
	...
	//$fail = $fail != '' ? 'RPT '.substr($fail,2): 'PASS';
	$take = $take_ignore == true? '' : take_courses_sessional($std_id, $l, $s);
	//$rept = $carryf == $rept? '': $rept;
	$carryf = $carryf != '' ? 'CARRY F '.substr($carryf,2)."<br>" : '';
	$rept = $rept != '' ? 'RPT '. substr($rept,2) : '';
	$rept = $take != '' ? 'TAKE '. $take ."<br>".$rept : $rept;
...
function take_courses_sessional($stdid, $l, $s) 
{
	$fos = std_course( $stdid );
	//$sql = 'Select stdcourse_custom2 From course_reg Where std_id='.$stdid.' && clevel_id='.$l.' && cyearsession='.$s.' && thecourse_id NOT IN ( Select stdcourse_id From students_results Where std_id='.$stdid.' && level_id='.$l.' && std_mark_custom2='.$s.' )';
	$sql = 'Select course_code From all_courses Where course_custom2 = '.$fos.' && level_id='.$l.' && course_custom5='.$s.' && course_status = "C" && thecourse_id NOT IN ( Select stdcourse_id From students_results Where std_id='.$stdid.' && level_id='.$l.' && std_mark_custom2='.$s.' && result_flag='.Sessional.' )';
	$q = mysqli_query($GLOBALS['connect'], $sql );
	$take = '';
	if (0!=mysqli_num_rows($q)) {
		while ($r = mysqli_fetch_assoc($q)){
