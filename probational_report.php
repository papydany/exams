...
if( $l > 1 )
	{
		...
		$set['rpt'] = array(0=>'<th>PROBATIONAL COURSES</th>', 1=>'<th></th>', 2=>'<th class="tB"></th>');
		$set['carry'] = array(0=>'<th>CARRY OVER COURSES</th>', 1=>'<th></th>', 2=>'<th class="tB">CH</th>');
		$set['cpga'] = array(0=>'<th>CGPA</th>', 1=>'<th></th>', 2=>'<th class="tB"></th>');
		$set['chr'] = array(1=>'', 2=>'');
		$set['plus'] = 0;
...
foreach( $std_off_list as $ind_std ) {
	...
	echo '<tr>';
		echo '<td>',$c,'</td>',
			...
			//'<td class="s9">', prob_get_carry_over_courses( $s, $ind_std['std_id'], $l, $d, $p, $fos ),'</td>';
...
else
{
   $ec="++";
	    ...
	    echo !empty($cod[2]) ? '<td></td>' : '';
		 /**/
		if (($f == 6) || ($d == 25)) // FACULTY OF AGRIC 
		{
		echo '<td class="s9"><div class="dw">',result_check_pass_2_probation($l, $ind_std['std_id'], $s, $cgpa),
		'</div></td>';
	}else{
