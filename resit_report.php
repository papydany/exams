...
//report design setting
	...
	if($special ){
		...
		//$set['chr'] = array(1=>'', 2=>'');
		//$set['carry'] = array(0=>'<th></th>', 1=>'<th></th>', 2=>'<th class="tB"></th>');
		}else{
		$set['rpt'] = array(0=>'', 1=>'', 2=>'');
		$set['carry'] = array(0=>'', 1=>'', 2=>'');
		}
	...
	if( $special ){
		...
		$set['class'] = array(0=>'<th>CLASS OF<br /> DEGREE</th>', 1=>'<th></th>', 2=>'<th class="tB"></th>');
...
if( $l > 11 ){
		$mysem1 = $aa+$set['plus']+1;
		$mysem2 = $ab+$set['plus']+1;
		$myspace = '';
		}else{
		$mysem1 = $aa+$set['plus']+1;
		$mysem2 = $ab+$set['plus']+1;
	...
	'<thead>',
		'<tr class="thead">',
			...
			'<th>REG NO</th>',
	 		$set['rpt'][0],
			$set['carry'][0],
		 '<th colspan="',$mysem1,'">FIRST SEMESTER RESULTS</th>',
		 '<th colspan="',$mysem2,'">SECOND SEMESTER RESULTS</th>',
		 	$myspace,
			'<th>GPA</th>',
			$set['cpga'][0],
			$set['class'][0],
...
//$c++; Display of previous results ** This is actually the current result
$c++;
	...
	echo '<tr>';
		echo '<td>',$c,'</td>',
			 ...
			 //if($l>11)
				...
				if( $aa != 0 || $ab != 0 ) {
					...
					//if( $l > 11) // 11
						//echo '<td class="tB s9">',get_fake_chr_resit( 1, $rpt_list, $carryov_list, $s, $ind_std['std_id'] ),'</td>';
					if( $l == 11 || $l== 12){
					...
					$electives = std_elective_result_resit( $ind_std['std_id'], $s, $l, $fos );
					...
					for($i=0; $i<$k; $i++) {
						...
						if( $i == ($aa + 1) ) {
							if( $l > 11 ) { //11
								...
								//echo '<td class="tB s9">',get_fake_chr_resit( 2, $rpt_list, $carryov_list, $s, $ind_std['std_id'] ),'</td>';
							}
						}
						else {
		...
		//echo '<td>',get_gpa_resit($s, $ind_std['std_id']),'</td>';
				echo '<td>',get_gpa($s, $ind_std['std_id']),'</td>';
				$cgpa = auto_cgpa($s, $ind_std['std_id'], $l, $c_duration, $year_of_study);
				//$cgpa = get_cgpa_resit($s, $ind_std['std_id']);
				$finalyear = ( $l == $c_duration ) ? true : false;
				
				if(( $l > 1 ) && ($level_reps[$l] < $c_duration))
				$remark = get_remarksD1($p, $f, $d, $l, $s, $ind_std['std_id'], $cgpa, $fos, $finalyear );
				
				if(( $l > 11 ) && ($level_reps[$l] == $c_duration))
				$remark = get_remarksD1($p, $f, $d, $l, $s, $ind_std['std_id'], $cgpa, $fos, $finalyear );
				
				if(( $l > 11 ) && ($level_reps[$l] == $c_duration))
				$remark = str_replace("RPT","PASS<br/>Carry F ",$remark);
				...
				if(( $l > 11 ) && ($level_reps[$l] == $c_duration))
					echo '<td class="B tc">',G_degreed1($cgpa, $ignore),'</td>';
			...
			if(( $l == 11 ) && ($level_reps[$l] < $c_duration))
				$remark = str_replace("PASS<br/>","",$remark);
...
//Display of Current results // This is actually the previous result
...
//	$c++;
	...
	echo '<tr>';
		echo '<td>Prev</td>',
			 ...
			 if( $l >= 11 )
			{
				...
				if( $aa != 0 || $ab != 0 ) {
					...
					$regdcourses = fetch_Ireg($ind_std['std_id'], $s, $l);
					$first_semester = fetch_student_RESULT_resit($ind_std['std_id'], $arr1, $s, $regdcourses);
					$first_semester = empty($first_semester) ? array('') : $first_semester;
					$second_semester = fetch_student_RESULT_resit($ind_std['std_id'], $arr2, $s, $regdcourses);
					$second_semester = empty($second_semester) ? array('') : $second_semester;
					$ll = array_merge($first_semester, array(1=>array()), array(1=>array()), $second_semester, array(1=>array()) );
		...
		$cgpa = get_cgpa_resit($s, $ind_std['std_id']);
				...
				if(( $l == 11 ) && ($level_reps[$l] < $c_duration))
				$remark = str_replace("PROBATION","",$remark);
				
				if(( $l > 11 ) && ($level_reps[$l] == $c_duration))
				$remark = str_replace("WITHDRAW","",$remark);
		...
		echo '<td class="s9"><div class="dw">',$grc1,'</div></td>';
			}*/
		echo '<td class="s9"><div class="dw">', $remark,'</div></td>';
		
		//echo '<td class="s9"><div class="dw"></div></td>';
