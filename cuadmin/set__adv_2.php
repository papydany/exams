...
<?php
	...
	if( !isset($_GET['s'], $_GET['m']) ) {
		exit("This Action Cannot Continue");
	}
	...
	echo '<div style="padding:0 5px 10px; font-size:17px; font-family:arial"><b>',$fullname['surname'],'</b> ',$fullname['othernames'],'</div>';
	...
	if( 0!=mysqli_num_rows($regd) ) {
		...
		foreach( $ys as $y ) {
			...
			echo '<div style="display:block; width:100%; overflow:hidden; margin-bottom:10px;">',
					'<div style="float:left; width:10%; font-size:15px;">Level ',$level_id,'00<span style="display:block; font-size:11px; overflow:hidden">',$session,'</span></div>',
