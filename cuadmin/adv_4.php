...
$examofficer =$_SESSION['myexamofficer_id'];
	...
	if( isset($_POST['submit']) && $_POST['submit'] == 'Save Result' ):
			...
			if( !empty($v) ) {
				...
				if( 6 == $size ) {
					...
					if( 0 == mysqli_num_rows($s_) ) {
						...
						$qB .= 'INSERT INTO `students_results`(`std_id`,  `matric_no`,  `level_id`,  `stdcourse_id`, `std_mark`,  `std_grade`,  `cu`,  `cp`,  `std_cstatus`,  `std_mark_custom1`,  `std_mark_custom2`, `period`,`examofficer`,`date_posted` ) VALUES ('.$std_id.', "'.$matric_no.'", '.$lvl.', '.$courseid.', "'.$x['mark'].'", "'.$v.'", '.$cu.', '.$x['cp'].', "YES", "'.$sem.'", "'.$session.'", "'.$period.'","'.$examofficer.'","'.$date_posted.'");';
	...
	if( isset($_POST['submit']) && $_POST['submit'] == 'Delete Result' ):
		...
		//mysqli_query( $GLOBALS['connect'], 'Optimize table course_reg, students_results');
