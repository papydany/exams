		...
		switch( $_POST['submit'] ) {
			...
			case 'Save Changes':
				foreach( $_POST['opt'] as $k=>$v ) {
					$_ = mysqli_query( $GLOBALS['connect'], 'UPDATE exam_officers SET edit_allow_logon = '.$v.' WHERE eo_username = "'.$k.'"');
					if( mysqli_affected_rows($GLOBALS['connect'])>0 )
						$processed++;
				}
			break;
...
<div style="background:#666; font-size:18px; font-weight:700; font-family:arial; letter-spacing:1px; color:#FFF; padding:7px 10px;">Modify Edit Rights</div>
<?php
	
	$a = mysqli_query( $GLOBALS['connect'], 'SELECT `exam_officers`.`examofficer_id`, `exam_officers`.`department_id`, `exam_officers`.`fos`, `exam_officers`.`eo_username`, `exam_officers`.`eo_password`, `exam_officers`.`eo_title`, `exam_officers`.`eo_surname`, `exam_officers`.`edit_allow_logon`, `exam_officers`.`eo_firstname`, `exam_officers`.`eo_othernames`, departments.`departments_name` FROM `exam_officers` LEFT JOIN departments ON departments.`departments_id` = `exam_officers`.`department_id` WHERE `exam_officers`.`eo_username` NOT IN (\'cadmin\', \'super\') ORDER BY `exam_officers`.`eo_username`');
	...
	if( 0!=mysqli_num_rows($a) ) {
		...
		foreach( $list as $k=>$v ) {
			...
			echo '<ul>';
			echo '<li><select name="opt[',$v[0]['eo_username'],']"><option>',$v[0]['edit_allow_logon'],'</option><option>2</option><option>4</option><option>6</option></select></li>';
			echo '</ul>';
			echo '</li>';
