...
<?php
	...
	$list_courses_first = fetch_courses_verPROBATION( $d, $p, $l, 'first semester', $s, $f, $fos );
	$list_courses_sec = fetch_courses_verPROBATION( $d, $p, $l, 'second semester', $s, $f, $fos );
...
$std_off_list = fetch_student_mat_probational( $d, $p, $l, $f, $s, $fos );
	...
	echo '<tr>';
		echo '<td>',$c,'</td>',
			 ...
			 '<td>',$ind_std['matric_no'],'</td>',
		 	 '<td class="s9">',fetch_probation_courses($l, $s, $ind_std['std_id']),'</td>',
		...
		$cgpa = adv_get_cgpa($s, $ind_std['std_id'], $l);
		
		echo '<td>',get_gpa($s, $ind_std['std_id']),'</td>',
			 '<td class="B">',$cgpa,'</td>';
	    echo !empty($cod[2]) ? '<td></td>' : '';
		echo '<td class="s9"><div class="dw">',get_remarks_verPROBATION($s, $ind_std['std_id'],$l, $d, $cgpa, $p, $fos),'</div></td>',
			 '</tr>';		 
