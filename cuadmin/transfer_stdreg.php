...
<?php
	...
	if( isset($_GET['submit']) && !empty($_GET['department']) && !empty($_GET['level']) && !empty($_GET['course']) ):
		...
		if( 0==mysqli_num_rows($a) ) {
			...
			echo '<form action="viewstdreg.php" method="post">';
			...
			while( $r=mysqli_fetch_assoc($a) ) {
				...
				echo '<tr>',
					...
					'<td style="background:#EEE;"><input name="schk[]" type="checkbox" value="',$r['std_id'],'" /></td>',				
						...
						'<td style="text-align:center; background:#F5F5F5; border-right:none;"><a href="edit_reg_sem.php?std=',$r['std_id'],'">',$r['matric_no'],'</a></td>',
			...
			echo '</tbody></table><table class="us" style="background:#F1F1F1;"><tr><td style="text-align:center;"><input name="submit" type="submit" value="Edit Registration"></td></tr></table>';
