			...
			case 3:
				...
				if( isset($_POST['fos']) && !empty($_POST['fos']) ) {
					...
					foreach( $_POST['fos'] as $fos ) {
						...
						$values .= '( '.$prog.', '.$fac.', '.$dept.', "'.$fos.'", "'.$_POST['username'].'", "'.$_POST['password'].'", "","", "'.$_POST['fullname'].'", "",1,3,0),';					
					...
					if( 0==mysqli_num_rows($chk) ) {
						...
						$a = mysqli_query( $connect, 'INSERT INTO `exam_officers` (`programme_id`, `faculty_id`, `department_id`, `fos`, `eo_username`, `eo_password`, `eo_title`, `eo_surname`, `eo_firstname`, `eo_othernames`, `eo_status`, `mstatus`, `eo_course`) VALUES '. $values );
