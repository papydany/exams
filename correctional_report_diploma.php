...
<title>Diploma Correctional Report</title>
...
//report design setting
	...
	if($l > 1 ){ // $special
		...
		$set['cpga'] = array(0=>'<th>CGPA</th>', 1=>'', 2=>'');
		$set['chr'] = array(1=>'', 2=>'');
		$set['plus'] = 0;
		...
		$set['cpga'] = array(0=>'<th>CGPA</th>', 1=>'', 2=>'<th class="tB"></th>');
	...
	if( $special ){
		...
		$set['class'] = array(0=>'', 1=>'<th></th>', 2=>'<th class="tB"></th>');
		$set['dr'] = 'CORRECTIONAL DIPLOMA RESULTS';
...
//$c++;
	...
	echo '<tr>';
		...
		echo '<td>',get_gpa_corr($s, $ind_std['std_id']),'</td>';
				...
				$finalyear = ( $l == $c_duration ) ? true : false;
				$remark = get_remarks_corr($p, $f, $d, $l, $s, $ind_std['std_id'], $cgpa, $fos, $finalyear,$new );
