...
//report design setting
	...
	if( $special ){
		...
		$set['dr'] = 'UNDERGRADUATE INDUSTRIAL ATTACHMENT RESULTS';
	...
	echo '<tr>';
		echo '<td>',$c,'</td>',
			 ...
			 if( $l > 1 )
			 {
				...
				//$carryov_list = array();
				$grc =	get_repeat_courses_reworked_agric_IT($l, $s, $ind_std['std_id'], $d,$f);
				//$grc = get_repeat_courses_111($l, $s, $ind_std['std_id'], $d);
...
function result_check_it($l, $std_id)
{	$fail=''; $pass='';$c=0;
	$sql = 'Select stdcourse_id, cu From students_results Where std_id='.$std_id.' && level_id='.$l.' && std_grade IN ("F","N") GROUP BY stdcourse_id';
	...
	if (mysqli_num_rows($r)!=0){ // found failed courses in the level
		...
		{ $c++;
			...
			if (mysqli_num_rows($r1)!=0){ //found that failed course passed in the level
				...
				$sqlc = 'Select stdcourse_custom2 From course_reg Where thecourse_id='.$row['stdcourse_id'] .' && clevel_id='.$l.' && std_id='.$std_id;
