	...
	if(!$cc){
       if($cgpa <=1.5 || $fail_cu >=15){
		...
		foreach( $list_std as $k=>$v ) {
			...
			/*$cgpa = adv_get_cgpa(($c_sess - 1), $v, $c_lv);
			if (( $cgpa >= 0.75 && $cgpa < 1.00 ) && (isset($_POST['prob'])))
			{ // register normal failed courses and failed courses in next/probation year; session + 1
				...
				$sql .= ' UNION SELECT  `stdresult_id`,  `std_id`,  `matric_no`,  `level_id`,  `stdcourse_id`,  `std_ca`,  `std_exam`,  `std_mark`,  `std_grade`,  `cu`,  `cp`,  `std_cstatus`,  `std_mark_custom1`,  `std_mark_custom2`, `std_mark_custom3` FROM `students_results` WHERE std_id = '.$v.' && `level_id` = '.$c_lv.' && `std_grade` IN ("F","N") && `std_mark` >= 0 && `std_mark_custom2` = "'. ($c_sess + 1 ).'" ';
			} else { //original code*/
				$sql = 'SELECT  `stdresult_id`,  `std_id`,  `matric_no`,  `level_id`,  `stdcourse_id`,  `std_ca`,  `std_exam`,  `std_mark`,  `std_grade`,  `cu`,  `cp`,  `std_cstatus`,  `std_mark_custom1`,  `std_mark_custom2`, `std_mark_custom3` FROM `students_results` WHERE std_id = '.$v.' && `level_id` = '.$c_lv.' && `std_grade` IN ("F","N") && `std_mark` >= 0 && `std_mark_custom2` = "'.$c_sess.'" ';
			//}
